...
if($nivel_usuario == 'Corretor'){
	$query = $pdo->query("SELECT * FROM imoveis where status = 'Inativo' corretor = '$id_usuario' ORDER BY id desc");
}else{
	$query = $pdo->query("SELECT * FROM imoveis where status = 'Inativo' ORDER BY id desc");
}
...
//retirar aspas do texto do obs
	...
	echo 'Não possui nenhum Imóvel Inativo!';
}
